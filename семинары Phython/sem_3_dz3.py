...
count = 0
for i in word: # находит букву i в регистре
    if i in 'QWERTYUIOPASDFGHJKLZXCVBNM': # перебирает регистр
        for j in points_en: # перебирает регистр на английском языке
            if i in points_en[j]:
                count = count + j
    else:
        for j in points_en:
            if i in points_ru[j]: # перебирает регистр на русском языке
                count = count + j
print(count)